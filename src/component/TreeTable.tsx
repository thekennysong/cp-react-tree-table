...
export default class TreeTable extends Component<TreeTableProps, {}> {
  ...
  render() {
    ...
    Children.toArray(children).forEach((node: React.ReactNode) => {
...
const isColumnElement = (
  elem: any
): elem is React.ReactElement<ColumnProps> => {
  return checkElementType(elem, Column);
};

const checkElementType = <T extends {}>(
  elem: any,
  cmpType: React.ComponentType<T>
): elem is React.ReactElement<T> => {
  return (
    elem != null &&
    elem.type != null &&
